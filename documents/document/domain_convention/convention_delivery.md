...
- **Wire Tag Boundary (MANDATORY)**:
  - `json/form/binding` tags are allowed only in delivery DTOs (`presenters.go`, MQ presenters).
  - Usecase/domain structs must not carry wire tags.
  - Always map DTO -> usecase input through explicit mapper functions (`toInput()`, `toHandleCompletionInput()`, etc.).
