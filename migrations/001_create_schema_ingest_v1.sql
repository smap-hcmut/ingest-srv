...
-- CREATE SCHEMA IF NOT EXISTS schema_ingest;
-- ALTER SCHEMA schema_ingest OWNER TO ingest_master;

CREATE TYPE schema_ingest.source_type AS ENUM (
    'TIKTOK',
    'FACEBOOK',
    'YOUTUBE',
    'FILE_UPLOAD',
...
CREATE TYPE schema_ingest.source_category AS ENUM (
    'CRAWL',
    'PASSIVE'
);

CREATE TYPE schema_ingest.source_status AS ENUM (
    'PENDING',
    'READY',
    'ACTIVE',
    'PAUSED',
...
CREATE TYPE schema_ingest.onboarding_status AS ENUM (
    'NOT_REQUIRED',
    'PENDING',
    'ANALYZING',
    'SUGGESTED',
...
CREATE TYPE schema_ingest.dryrun_status AS ENUM (
    'NOT_REQUIRED',
    'PENDING',
    'RUNNING',
    'SUCCESS',
...
CREATE TYPE schema_ingest.crawl_mode AS ENUM (
    'SLEEP',
    'NORMAL',
    'CRISIS'
);

CREATE TYPE schema_ingest.job_status AS ENUM (
    'PENDING',
    'RUNNING',
    'SUCCESS',
    'PARTIAL',
...
CREATE TYPE schema_ingest.batch_status AS ENUM (
    'RECEIVED',
    'DOWNLOADED',
    'PARSED',
    'FAILED'
...
CREATE TYPE schema_ingest.publish_status AS ENUM (
    'PENDING',
    'PUBLISHING',
    'SUCCESS',
    'FAILED'
...
CREATE TYPE schema_ingest.trigger_type AS ENUM (
    'MANUAL',
    'SCHEDULED',
    'PROJECT_EVENT',
    'CRISIS_EVENT',
...
CREATE TYPE schema_ingest.target_type AS ENUM (
    'KEYWORD',
    'PROFILE',
    'POST_URL'
);

CREATE OR REPLACE FUNCTION schema_ingest.set_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
BEFORE UPDATE ON schema_ingest.data_sources
FOR EACH ROW
...
BEFORE UPDATE ON schema_ingest.crawl_targets
FOR EACH ROW
EXECUTE FUNCTION schema_ingest.set_updated_at();

-- FK back-references from dryrun_results / scheduled_jobs / external_tasks → crawl_targets
ALTER TABLE schema_ingest.dryrun_results
    ADD CONSTRAINT fk_dryrun_results_target
    FOREIGN KEY (target_id) REFERENCES schema_ingest.crawl_targets(id) ON DELETE SET NULL;

ALTER TABLE schema_ingest.scheduled_jobs
...
ALTER TABLE schema_ingest.data_sources
    ADD CONSTRAINT fk_data_sources_dryrun_last_result
    FOREIGN KEY (dryrun_last_result_id)
    REFERENCES schema_ingest.dryrun_results(id);

CREATE INDEX idx_data_sources_project_deleted
    ON schema_ingest.data_sources (project_id, deleted_at);

CREATE INDEX idx_data_sources_status_category
    ON schema_ingest.data_sources (status, source_category);

CREATE INDEX idx_data_sources_project_source_type
    ON schema_ingest.data_sources (project_id, source_type);
...
CREATE UNIQUE INDEX idx_data_sources_webhook_id_unique
    ON schema_ingest.data_sources (webhook_id)
    WHERE webhook_id IS NOT NULL;

CREATE INDEX idx_data_sources_config_gin
    ON schema_ingest.data_sources USING GIN (config);

CREATE INDEX idx_data_sources_mapping_rules_gin
    ON schema_ingest.data_sources USING GIN (mapping_rules);

CREATE INDEX idx_dryrun_results_project_created_desc
    ON schema_ingest.dryrun_results (project_id, created_at DESC);

CREATE INDEX idx_dryrun_results_source_created_desc
    ON schema_ingest.dryrun_results (source_id, created_at DESC);

CREATE INDEX idx_dryrun_results_job_id
    ON schema_ingest.dryrun_results (job_id);

CREATE INDEX idx_scheduled_jobs_status_scheduled_for
    ON schema_ingest.scheduled_jobs (status, scheduled_for);

CREATE INDEX idx_scheduled_jobs_source_scheduled_for_desc
    ON schema_ingest.scheduled_jobs (source_id, scheduled_for DESC);

CREATE INDEX idx_scheduled_jobs_project_scheduled_for_desc
    ON schema_ingest.scheduled_jobs (project_id, scheduled_for DESC);

CREATE INDEX idx_external_tasks_source_created_desc
    ON schema_ingest.external_tasks (source_id, created_at DESC);
...
CREATE INDEX idx_external_tasks_status_created_desc
    ON schema_ingest.external_tasks (status, created_at DESC);

CREATE INDEX idx_raw_batches_source_received_desc
    ON schema_ingest.raw_batches (source_id, received_at DESC);
...
CREATE INDEX idx_raw_batches_publish_status_received_desc
    ON schema_ingest.raw_batches (publish_status, received_at DESC);

CREATE INDEX idx_raw_batches_checksum
    ON schema_ingest.raw_batches (checksum)
    WHERE checksum IS NOT NULL;

CREATE INDEX idx_crawl_mode_changes_source_triggered_desc
    ON schema_ingest.crawl_mode_changes (source_id, triggered_at DESC);

CREATE INDEX idx_crawl_mode_changes_project_triggered_desc
    ON schema_ingest.crawl_mode_changes (project_id, triggered_at DESC);

-- crawl_targets indexes
CREATE INDEX idx_crawl_targets_data_source
    ON schema_ingest.crawl_targets (data_source_id);

CREATE INDEX idx_crawl_targets_next_crawl_active
    ON schema_ingest.crawl_targets (next_crawl_at)
    WHERE is_active = true;

CREATE INDEX idx_crawl_targets_type
    ON schema_ingest.crawl_targets (data_source_id, target_type);

-- target_id indexes on referencing tables
CREATE INDEX idx_dryrun_results_target
    ON schema_ingest.dryrun_results (target_id)
    WHERE target_id IS NOT NULL;

CREATE INDEX idx_scheduled_jobs_target
    ON schema_ingest.scheduled_jobs (target_id)
    WHERE target_id IS NOT NULL;

CREATE INDEX idx_external_tasks_target
    ON schema_ingest.external_tasks (target_id)
    WHERE target_id IS NOT NULL;
